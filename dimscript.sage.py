...
def build_sum(k, s, m, n, base_field=F):
    if k <= _sage_const_0  or s <= _sage_const_0  or m <= _sage_const_0  or n <= _sage_const_0 :
    ...
    for i in range(n_rows):
        for j in range(n_cols):
            val = M[i, j]
            if val != _sage_const_0  and val not in seen and (-val) not in seen:
                seen.add(val)
                unique_nonzero.append(val)
    M_flat = Matrix(P, len(unique_nonzero), _sage_const_1 , unique_nonzero)
...
for (s, k) in sorted(iter.product(range(_sage_const_2 , _sage_const_6 ), range(_sage_const_2 , _sage_const_6 )), key=lambda x: x[_sage_const_0 ] * x[_sage_const_1 ]):
    for n in range((k * s - k + _sage_const_1 ) >> _sage_const_1  << _sage_const_1 , k * s + _sage_const_1 , _sage_const_2 ):
        if n <= k: 
            continue
        print("--")
        if n > _sage_const_16 :
            continue
        m_upper_bound = (n + _sage_const_3 ) // _sage_const_2 
        m_lower_bound = _sage_const_1  #  max(m_upper_bound - k - 2, 1)
        for m in range(m_lower_bound, m_upper_bound + _sage_const_1 ):
            ...
            expected_full = (k - _sage_const_1 ) * n - (k - _sage_const_1 ) * k / _sage_const_2 
           
            expected_dim = (_sage_const_2  * k - _sage_const_3  + _sage_const_1 ) * m - max(_sage_const_2  * m + k - n, _sage_const_0 ) * (_sage_const_2  * m + k - n - _sage_const_1 ) / _sage_const_2  + max(_sage_const_0 , _sage_const_2  * m - n) * (_sage_const_2  * m - n + _sage_const_1 ) / _sage_const_2  
